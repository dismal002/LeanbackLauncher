...
class ActiveDayBuffer {
    ...
    public void set(Date date, Signals value) {
        this.mBuffer.put(Util.getDay(date), value);
    ...
    public Signals get(Date date) {
        return this.mBuffer.get(Util.getDay(date));
    }
    ...
    public double getAggregatedScore(Aggregator<Signals> aggregator) {
        ...
        if (this.mBuffer.size() == 0) {
            this.mScore = Ranker.GROUP_STARTER_SCORE;
            this.mDirty = false;
            return this.mScore;
        }
        for (int i = 0; i < this.mBuffer.size(); i++) {
            aggregator.add(Util.getDate(this.mBuffer.keyAt(i)), this.mBuffer.valueAt(i));
        }
